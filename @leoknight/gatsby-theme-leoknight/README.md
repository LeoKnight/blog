...
yarn add react react-dom gatsby @leoknight/gatsby-theme-leoknight
```
...
### Step 3: Using `@leoknight/gatsby-theme-leoknight`

You must add `@leoknight/gatsby-theme-leoknight` as a plugin in `gatsby-config.js`.
`@leoknight/gatsby-theme-leoknight` is configurable through plugin options but we will use the defaults for now.

```js
// gatsby-config.js

module.exports = {
  plugins: ['@leoknight/gatsby-theme-leoknight'],
};
```

---

Once you've setup `@leoknight/gatsby-theme-leoknight` plugin in `gatsby-config.js` you can start creating your first Posts. In order to create a Post you also need at least one Author.
...
Once you have your project setup with `gatsby-source-contentful` and `@leoknight/gatsby-theme-leoknight` and your Contentful space is setup with the imported model you are good to go.
...
import novelaTheme from '@leoknight/gatsby-theme-leoknight/src/gatsby-plugin-theme-ui';
